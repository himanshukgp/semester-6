...
void simplex_iteration(int m, int n, vector<vector<double> > &table, vector<vector<double> > &iter2, 
		vector<int> &basic_variable1, vector<int> &basic_variable2, vector<double> &min_ratios){
	int min_column=0;
	...
	double pivot_ele = table[pivot_i][pivot_j];
...
{ 
	...
	// TO DOS
	// 1. TAKE INPUT AND CONFIRM
	// 2. MANIPULATE EACH OF THE VARIABLES
	// 3. MANIPULATE d
	// 4. PERFORM SIMPLEX
	...
	int number_of_surplus = 0;
	for(int i=0;i<sign.size();i++){
		if(sign[i]==-1) number_of_surplus++;
	}

	vector<vector<double> > new_table(m+1, vector<double> (n+number_of_surplus+1));
	...
	for(int i=0;i<m;i++){
		new_table[i][new_table[0].size()-1] = new_table[i][table[0].size()-1];
		new_table[i][table[0].size()-1] = 0;
	}
	...
	for(int i=0;i<m;i++){
		if(sign[i]==-1){
			for(int j=0;j<new_table[0].size();j++){
				new_table[m][j] = new_table[m][j] - BIGM * new_table[i][j];
			}
		}
	}
	...
	vector<vector<double> > iter2(m+1, vector<double>(n+1));
	vector<double> min_ratios(m);
	vector< vector<vector<double> > > result_tables;
	result_tables.push_back(table);
	vector<vector<int> > result_basic_variables;
	result_basic_variables.push_back(basic_variable1);
	vector<vector<double> > result_min_ratios;
	int iteration=0;


	while(1){
		iteration++;
		//cout << " ++++ Iteration ongoing ----- : Iteration number   " << iteration << endl; 

		simplex_iteration(m, n, table, iter2, basic_variable1, basic_variable2, min_ratios);

		result_tables.push_back(iter2);
		result_basic_variables.push_back(basic_variable2);
		result_min_ratios.push_back(min_ratios);

		int flagab=1;
		for(int i=0;i<n;i++){
			if(min_ratios[i]>0) flagab=0;
		}
		if(flagab){
			cout << "***   UNBOUNDED   ***";
			break;
		...
		int flag=1;
		for(int i=0;i<n;i++){
			if(iter2[m][i] < 0) flag=0;
		}
		if(flag) break;

		table = iter2;
		basic_variable1 = basic_variable2;
	}


	// for(int i=0;i<result_tables.size();i++){
	// 	for(int j=0;j<result_tables[i].size();j++){
	// 		for(int k=0;k<result_tables[i][j].size();k++){
	// 			cout << result_tables[i][j][k] << "\t";
	// 		}
	// 		cout << endl;
	// 	}
	// 	cout << endl ;
...
cout << "Input 2 for Number of Iterations to solve the problem" << endl;
cout << "Input 3 Non-Basic variables with evaluations" << endl;
cout << "Input 4 for List of Basic variables along with min ratios in ith iteration" << endl;
cout << "Input 5 for simplex table of i th iteration" << endl;
cout << "Input 6 for optimal value" << endl;
...
while(1){
	...
	if(input_value==1){
		cout << endl << "Basic Feasible Solution" << endl;
		...
		cout  << endl << "Number of Iterations to solve the problem" ;
		cout << "Ans   =    " << iteration << endl;
	}

	else if(input_value==3){
		int p;
		cout << "Enter 1 for 1st iteration and so on .... =  ";
		cin >> p;

		cout  << endl << "Non-Basic variables with evaluations at iteration" << p << " is  " <<  endl;

		for(int i=0;i<n;i++){
			if(result_basic_variables[p-1][i]<=n){
				cout << "x" << result_basic_variables[p-1][i] << "\t";
			}
			else{
				cout << "z" << result_basic_variables[p-1][i]-n << "\t";
			}
		}
		cout << endl;
		for(int i=0;i<n;i++){
			cout << result_tables[p-1][m][i] << "\t";
		}
		cout << endl;
	}
		...
		cout << endl << "List of Basic variables along \n \t with min ratios in ith iteration " << p << " is  " <<  endl;
		...
		cout << endl << "simplex table of i th iteration " << endl;

		for(int i=0;i<result_tables[p].size();i++){
			for(int j=0;j<result_tables[p][i].size();j++){
				cout << result_tables[p][i][j] << "\t";
			}
			cout << endl;
		}
		cout << endl;
		...
		cout << endl << "optimal solution =    " << result_tables[result_tables.size()-1][m][n];
		cout << endl;
	}
